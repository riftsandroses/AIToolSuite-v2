...
# ... (FileUploadVulnerabilityScanner class remains the same) ...
...
class ChatGPTAnalyzer:
    ...
    # NEW: Added a robust JSON parser to handle markdown fences
    def _safe_json_parse(self, raw_output: str):
        if not raw_output:
            return None
        ...
        if match:
            try:
                ...
                return None
        logger.error(f"No JSON object found in ChatGPT output: {raw_output}")
        return None
    ...
    def analyze_api_for_file_download(self, api_data):
        ...
        try:
            ...
            # UPDATED: Use the new safe parser
            parsed_json = self._safe_json_parse(content)
            if parsed_json:
                return parsed_json
            else:
                logger.error(f"Invalid JSON from ChatGPT after cleaning: {content}")
...
class FileDownloadTester:
    ...
    def test_file_download(self, api, analysis_result):
        ...
        test_count = getattr(settings, 'FILE_DOWNLOAD_TEST_COUNT', 10)
        success_count = 0
        
        # UPDATED: Safely handle headers, params, and body
        headers = api.headers if isinstance(api.headers, dict) else json.loads(api.headers or '{}')
        headers['Authorization'] = f"Bearer {access_token}"
        
        params = api.query_params if isinstance(api.query_params, dict) else json.loads(api.query_params or '{}')
        body = api.body if isinstance(api.body, dict) else json.loads(api.body or '{}')
        ...
        for i in range(test_count):
            ...
            try:
                ...
                if api.method.upper() not in ['GET', 'HEAD']:
        ...
        is_vulnerable = success_rate > threshold
...
class ScanOrchestrator:
    def __init__(self, scan_id, chatgpt_key):
        ...
        self.history = None
        self.stats = None # Keep this

    def initialize_scan(self):
        try:
            # Fetch the objects created by the view
            self.history = ScanHistory.objects.get(scan_id=self.scan_id)
            self.stats = ScanStats.objects.get(scan_id=self.scan_id)
            
            # --- FIX: Only check and update history status ---
            if self.history.status != 'running':
                self.history.status = 'running'
                self.history.save()
                
        except ScanHistory.DoesNotExist:
            logger.error(f"Scan {self.scan_id}: ScanHistory object not found. The View must create it.")
            raise
        except ScanStats.DoesNotExist:
            logger.error(f"Scan {self.scan_id}: ScanStats object not found. The View must create it.")
            raise
    ...
    def complete_scan(self):
        ...
        vulnerable_count = FileDownloadTest.objects.filter(scan_id=self.scan_id, is_vulnerable=True).count()
        total_tests = FileDownloadTest.objects.filter(scan_id=self.scan_id).count()
        avg_success = FileDownloadTest.objects.filter(scan_id=self.scan_id).aggregate(avg_success=Avg('success_rate'))['avg_success'] or 0
        
        # Update stats object (but NOT its status)
        if self.stats:
            self.stats.total_tests = total_tests
            self.stats.total_vulnerabilities = vulnerable_count
            self.stats.avg_success_rate = avg_success
            # --- FIX: Removed status update line ---
            self.stats.save()
    
    def log_message(self, message):
        if self.history:
            # Only save if the message is new to avoid too many DB hits
            new_log = f"{timezone.now().isoformat()} - {message}\n"
            if new_log not in self.history.logs:
                self.history.logs += new_log
                self.history.save(update_fields=['logs'])
        logger.info(f"Scan {self.scan_id}: {message}")
    
    def run_scan(self):
        try:
            self.initialize_scan() # This now fetches the objects
            self.log_message("Scan started")
            
            apis = PostmanAPI.objects.filter(scan_id=self.scan_id)
            
            if not apis.exists():
                self.log_message("No APIs found for this scan_id. Completing scan.")
                self.complete_scan()
                return True

            self.history.total_apis = apis.count()
            self.history.save(update_fields=['total_apis'])
            ...
            for api in apis:
                try:
                    ...
                    api_data = {
                        ...
                        "headers": safe_load(api.headers),
                        "body": safe_load(api.body),
                        "query_params": safe_load(api.query_params),
                        "authorization": safe_load(api.authorization)
                    }
                    
                    analysis_result = self.chatgpt_analyzer.analyze_api_for_file_download(api_data)
                    self.log_message(f"Analysis result for API {api.id}: {analysis_result.get('reason')}")
                    ...
                    if analysis_result.get('potential_file_download', False):
                        ...
                        # This part was already correct in your file, which is good!
                        FileDownloadTest.objects.create(
                            scan_id=self.scan_id, api_id=api.id, api_name=api.name, url=api.url,
                            is_vulnerable=is_vulnerable, success_rate=success_rate,
                            test_count=getattr(settings, 'FILE_DOWNLOAD_TEST_COUNT', 10),
                            success_count=int(success_rate * getattr(settings, 'FILE_DOWNLOAD_TEST_COUNT', 10)),
                            details={"analysis": analysis_result, "request": api_data}
                        )
                    ...
                    self.history.save(update_fields=['tested_apis', 'vulnerable_apis'])
            ...
            self.complete_scan() # This now updates the 'completed' status
            return True
            
        except Exception as e:
            self.log_message(f"Scan failed: {str(e)}")
            # --- FIX: Update history status, remove stats status ---
            if self.history:
                self.history.status = 'failed'
                self.history.save(update_fields=['status'])
            # No self.stats.status to update
            return False
# ... (Rest of the services.py file remains unchanged) ...
