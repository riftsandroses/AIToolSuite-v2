...
from .serializers import (
    ...
    ScanHistorySerializerTC3, ScanStatsSerializerTC3,VulnerabilitySummaryItemSerializerTC3
)
            ...
            return Response(
                {
                    ...
                    'message': f'Scan {scan_id} has not been initiated.'
                },
...
class ScanStatusViewTC2(APIView):
    ...
    def get(self, request, scan_identifier):
        try:
            # Check if the identifier is a UUID or an integer ID
            try:
                # Attempt to validate it as a UUID
                uuid.UUID(str(scan_identifier))
                # If valid, query by the primary key (id)
                scan = ScanTC2.objects.get(id=scan_identifier)
            except ValueError:
                # If not a valid UUID, assume it's the integer scan_id
                scan = ScanTC2.objects.get(scan_id=scan_identifier)
            ...
            return Response(
                {
                    'status': 'NOT_INITIATED',
                    'message': f'Scan with ID {scan_identifier} has not been initiated.'
                },
                status=status.HTTP_200_OK
            )
class ScanResultsViewTC2(APIView):
    ...
    def get(self, request, scan_identifier):
        try:
            uuid.UUID(str(scan_identifier))
            scan = get_object_or_404(ScanTC2, id=scan_identifier)
        except ValueError:
            scan = get_object_or_404(ScanTC2, scan_id=scan_identifier)
...
class ScanHistoryViewTC2(APIView):
    ...
    def get(self, request, scan_identifier=None):
        if scan_identifier:
            try:
                # We'll use .get() which raises DoesNotExist, instead of get_object_or_404
                try:
                    uuid.UUID(str(scan_identifier))
                    scan = ScanTC2.objects.get(id=scan_identifier)
                except ValueError:
                ...
                # If the scan is not found, return the custom 202 response
                return Response(
                    {'detail': 'No ScanTC2 matches the given query.'}, 
                    status=status.HTTP_202_ACCEPTED
                )
...
class CancelScanViewTC2(APIView):
    ...
    def post(self, request, scan_identifier):
        try:
            uuid.UUID(str(scan_identifier))
            scan = get_object_or_404(ScanTC2, id=scan_identifier)
        except ValueError:
...
class VulnerabilitySummaryAPIViewTC3(APIView):
    ...
    def get(self, request, scan_id=None):
        ...
        serializer = VulnerabilitySummarySerializerTC3(instance=summary_data)
