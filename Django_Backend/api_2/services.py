        ...
        try:
            with transaction.atomic():
                ...
                if not created:
                    ...
                    scan_session.started_at = django_timezone.now()
                    scan_session.save()
    ...
    def _perform_scan(self, scan):
        ...
        try:
            ...
            for i, api in enumerate(apis, 1):
                try:
                    # Each API scan in its own atomic transaction.
                    # If this fails, the 'except' block is executed AFTER the rollback.
                    with transaction.atomic():
                    ...
                    # The transaction for the failed API has been rolled back.
                    # Now we can safely log the error to the database in a new transaction.
                    logger.error(f'Transaction failed for API {api["id"]}: {str(e)}')
                    try:
                        self._log_scan_event(scan, 'error', f'Failed to scan API {api["id"]}: {str(e)}')
                    except Exception as log_error:
                        # If logging also fails, use the standard logger as a fallback.
                        logger.error(f'CRITICAL: Failed to log scan error to database for API {api["id"]}: {log_error}')
                    
                    # Continue with the next API in the loop.
                    continue
            ...
            try:
                ...
                logger.error(f"Failed to mark scan as failed: {cleanup_error}")
