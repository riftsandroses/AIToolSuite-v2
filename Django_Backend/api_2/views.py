...
class ScanStatusTC2View(APIView):
    ...
    def get(self, request, scan_id):
        try:
            ...
            return Response(
                {'error': 'Scan not found', 'status': 'not_found'}, 
                status=status.HTTP_200_OK
            )
    ...
    def get(self, request, scan_id=None):
        if scan_id:
            try:
                ...
                return Response(
                    {'error': 'Scan summary not found', 'status': 'not_found'}, 
                    status=status.HTTP_200_OK
                )
        else:
...
class JWTScanDetailAPIViewTC4(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            instance = self.get_queryset().get(scan_id=scan_id)
            serializer = self.get_serializer(instance)
            return Response(serializer.data, status=status.HTTP_200_OK)
        except JWTScanTC4.DoesNotExist:
            # If it doesn't exist, return a 200 OK with an error message
            return Response(
                {'detail': f'No JWTScanTC4 matches the given query for scan_id: {scan_id}.'},
                status=status.HTTP_200_OK
            )
