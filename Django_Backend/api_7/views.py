...
class ExportSSRFResultsView(APIView):
    """
    Exports SSRF analysis results for a given scan.
    """
    authentication_classes = [JWTAuthentication]
    permission_classes = [permissions.IsAuthenticated]
    ...
    def get(self, request, scan_id):
        ...
        # Use the same logic from _get_scan_history to fetch data
        candidates = SSRFCandidate.objects.filter(api__scan=scan).order_by('-risk_score')
        llm_analyses = LLMAnalysisResult.objects.filter(ssrf_candidate__api__scan=scan)
        proofs = SSRFProof.objects.filter(ssrf_candidate__api__scan=scan)
        ...
        response = JsonResponse(export_data, safe=False, json_dumps_params={'indent': 2})
        response['Content-Disposition'] = f'attachment; filename="ssrf_scan_{scan_id}_results.json"'
        return response

class SSRFScanSummaryView(APIView):
    """
    Get compact summary for scan results
    """
    authentication_classes = [JWTAuthentication]
    permission_classes = [permissions.IsAuthenticated]
    ...
    def get(self, request, scan_id):
        ...
        candidates = SSRFCandidate.objects.filter(api__scan=scan)
        llm_analyses = LLMAnalysisResult.objects.filter(ssrf_candidate__api__scan=scan)
        proofs = SSRFProof.objects.filter(ssrf_candidate__api__scan=scan)

        confirmed_candidates = llm_analyses.filter(is_ssrf_candidate=True).values_list('ssrf_candidate_id', flat=True)

        last_analysis = candidates.order_by('-heuristic_completed_at').first()

        return Response({
            "scan_id": scan.id,
            "scan_name": scan.scan_name,
            "status": "completed" if candidates.exists() else "pending",
            "statistics": {
                "total_candidates": candidates.count(),
                "confirmed_vulnerabilities": len(confirmed_candidates),
                "proofs_generated": proofs.count(),
                "highest_risk_score": candidates.aggregate(max_risk=Max('risk_score'))['max_risk'] or 0,
                "last_analysis": last_analysis.heuristic_completed_at if last_analysis else None
            }
