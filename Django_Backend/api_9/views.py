...
class DiscoverEndpointsView(APIView):
    ...
    def get(self, request):
        """
        Get existing unlisted endpoints for a scan_id
        """
        ...
        try:
            endpoints = UnlistedEndpoints.objects.filter(scan_id=scan_id)
            serializer = UnlistedEndpointsSerializer(endpoints, many=True)
            return Response({
                'scan_id': scan_id,
                'endpoints_count': endpoints.count(),
                'endpoints': serializer.data # We will use this 'endpoints' key in React
            }, status=status.HTTP_200_OK)
            
        except Exception as e:
            logger.error(f"Error fetching endpoints for scan_id {scan_id}: {e}")
            return Response(
