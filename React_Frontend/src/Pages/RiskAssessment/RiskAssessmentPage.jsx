...
import StaticRiskAnalysis from "../../Components/RiskAssessment/StaticRiskAnalysis";
...
export default function RiskAssessmentPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    backend: "",
    database: "",
    tech_stack_file: null,
    sow_proposal_file: null, // --- ADDED ---
    architecture: "",
    hosting: "",
    sensitive_data: "",
  ...
  const combineTechStack = () => {
    // Note: This does not include the file, as your API setup
    // seems to only take text. This is fine for the UI.
  ...
  const prepareFormDataForSubmission = () => {
    // Note: This function doesn't handle the file upload (tech_stack_file).
    // If you need to upload the file to your API, this function
    // and handleSubmitAssessment will need to be changed to use FormData.
    // For now, it just shows the file name in the UI as requested.
    return {
  ...
  const handleSubmitAssessment = async () => {
    setIsSubmitting(true);
    try {
      // --- This is simplified for the static demo ---
      // We just move to the next step without any API calls.

      setIsEditing(false); // Reset editing mode
      setActiveStep(3); // Move to Risk Analysis step

    } catch (error) {
      console.error("Error during submit step:", error);
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleAssessmentSelect = (assessment) => {
    ...
    setFormData({
      ...
      tech_stack_file: null,
      sow_proposal_file: null // Reset file when selecting an old assessment
    });
  ...
  const handleEditAssessment = (assessment, e) => {
    ...
    setFormData({
      ...
      database: techStackParts.database || "",
      tech_stack_file: null,
      sow_proposal_file: null // Reset file when editing
    });
  ...
  const handleNewAssessment = () => {
    ...
    setFormData({
      ...
      database: "",
      tech_stack_file: null,
      sow_proposal_file: null, // Reset file
      architecture: "",
      hosting: "",
      sensitive_data: "",
      third_party_integrations: "",
